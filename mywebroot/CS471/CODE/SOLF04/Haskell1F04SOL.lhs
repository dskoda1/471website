...
> newton number guess near = 
>     if ( abs(guess -(guess + number/guess)/2) < near) 
>       then (guess + number/guess)/2
>       else newton number ((guess + number/guess)/2) near
...
Write a function "isOrigin" that returns true if the point is the origin and false otherwise.  If the constructor for your Point is Pt then to execute you type:
...
> collinear (Pt x1 y1) (Pt x2 y2) (Pt x3 y3) = (x1 - x2)*(y2 - y3) == (y1 - y2)*(x2 -x3)
