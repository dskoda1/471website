> module Haskell1F03
>     where

YOUR NAME, EMAIL
Don't forget to rename file as defined in the instructions. You do not
have include above the "+++++" line in your hard copy.

Purpose:  Introduce Haskell 
            by programming
               simple recursive functions
               simple function application
               curried functions (partially evaluated functions)
            by examining types using experimentation 
            and to experience the literal style of programming.

          You do not need to use modules on this assignment.

Note: In the homework file put only one space between ">" and the code.  
The examples above use more than one space.  
Do not use a tab...  JUST one SPACE.

 A) Looking in around ...  experiments. Not to be handed in!
     (After opening hugs you will get a prompt 
      Prelude><your type here if you clicked on .lhs file> )
      a) What does ...> :? <return> do?
      b) What does :t isAscii <return> do?
             A :t head <return> ??
             B :t (:) <return> ??
             C :t []
  B) Continue to examine, via your text editor, 
     the Prelude.hs file that comes with your hugs. 
     What is the purpose of the file? 
     What is the most interesting function you find?  
     Do not turn in ... be prepared to discuss in class.  
     (Prelude.hs is in the lib directory.  Use a text editor to view.
      OR you can view http://haskell.org/onlinereport/prelude-index.html)
  C) Try on the command line 
   (You do not need to understand why everything works below.)
                 
      ...> fst (3 ,4)
      ...> max 9 5
      ...> snd (3,4)
      ...> [1 .. 12]
      ...> ['a' .. 'z']
      ...> splitAt 4 "asdbefg"
      ...> [ y*2 | y <- [2,4 .. 10]]
      ...> :t 1
      ...> :t [1,2,3]
      ...> :t (+)
      ...> :t "abc"
      ...> :t ['a','b']
      ...> :t "abc" ++ "abc"
      ...> :t div
      ...> div 6 4
      ...> [ 1 ..]  

+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
11/07
YOUR NAME, EMAIL
Assignment Haskell1.lhs

Problem 1: (easy)
The purpose of problems 1 and 2 is for you to see the difference between 
applying functions to constant pairs and applying functions to constants

(very easy) Write a Haskell script,  pyth(x,y), 
	    which returns the sum of  x*x and y * y
    ...> pyth(3,4)
    25 
      

    Code to follow this line.  Remember you must begin the code with ">"

> pyth(x,y) = 

  What is the derived type (signature) of pyth(x,y)?
  Write answer here.


Problem 2: (easy)

Rewrite problem one except instead you wont be using a pair

> pythC x y =  


    What is the derived type of pythC x y ?
    What is the type of pythC ?  
Given the following definition:

> pythP = pythC 4

What is the type of pythP?


Problem 3: (easy)
 Below is a recursive definition for computing x^n:
     power (a,n) = 1 if n =0
     power (a,n) = a if n =1
     power (a,n) = a*power(a,n-1) if n >1      
                         

 Complete the below Code:

> power :: Integer -> Integer -> Integer 
> power a 0 = 1
> 
> 


Problem 4: 
   Given a list of Integral, write a Haskell definition, addListWeird, 
   adds all the even numbers and doubles and adds the odd numbers.

 . You may use built in functions 
   such as "odd" or "even"

Complete the code below: (I have supplied the base case and type)

> addListWeird :: Integral a => [a]  -> a
> addListWeird [] = 0
>     




Problem 5: (easy)
The purpose of this simple problem is to illustrate currying (partially 
evaluated) functions.
The following function finds the maximum of three integers:

> maxOf3Ints :: Int -> Int -> Int -> Int
> maxOf3Ints x y z
>     | x >= y && x >= z = x
>     | y >= z           = y
>     | otherwise        = z 

Derive the following definitions using maxOf3Ints, 
Include as a comment the inferred type of the new function. 
You find out the type by the command  ":t".
   
 For example:
 Define maxOf17and2Ints which is the max of two numbers and 17  
 using maxOf3Ints definitions

maxOf999and2Ints :: Int -> Int -> Int

> maxOf999and2Ints = maxOf3Ints 999

      A) Define maxOf49and2Ints which is the max of two numbers and 49  
      (use maxOf3Ints).  (Place code BELOW --remember ">")
      CODE:
    



      B) Define maxOf49_8and1Int which is the max of one numbers 
         and 81 and 15
      (use maxOf49and2Ints).  (Place code BELOW --remember ">")
      CODE:


      C) Define numberSixteenHundredOne 
         which is the max of one numbers and 49, 8, 1601  
      (use maxOf49_8and1Int).  (Place code BELOW --remember ">")
      CODE:



Problem 6: (easy)     
The purpose of this problem is to illustrate different ways to express logical
expressions.
   The function "implies"  takes tow Boolean arguments and returns a
   Boolean Result.  
   Its truth table is:
      x       y         implies x y
      True    True         True
      True    False        False
      False   True         True
      False   False        True



 Define implies three ways:
   a. Using conditional expression (if..then...else)
Code:
 
> impliesC :: Bool -> Bool -> Bool



   b. Using guards 
  
Code:
 
> impliesG :: Bool -> Bool -> Bool


    c. Using a single Boolean expression 
       Boolean operators are:  not, &&, ||

Code:
 
> impliesE :: Bool -> Bool -> Bool



problem 7:

Complete the definition below such that maxMin returns a pair of Ords 
such that the first element in the pair is the maximum value in the list
and the snd element is the minimum value in the list. You will find the 
prelude functions,  max, min, fst and snd useful for finishing this definition.
Use function application for this code.

Haskell1F01> maxMin "ezZvawAF"
('z','A')
Haskell1F01> maxMin [4,6,9,3,11,0,123,8]
(123,0)

> maxMin [a] = (a,a)
> maxMin (a:as) = 


What is the type of maxMin




problem 8:

Define a Haskell function merge :: [Int] -> [Int] ->[Int] so that if the 
lists as and bs are ordered, merge as bs is an ordered list containing all
the elements of as and bs.


> merge :: [Int] -> [Int] -> [Int]




problem 9:
Use the function merge you defined in the preceding exercise to define 
merge sort function mSort :: [Int] -> [Int].  You will find the prelude
function splitAt useful for splitting the list to be sorted into smaller lists.
Note div is a prefix operator.  
You may assume that the input list is not empty to start.


Problem 10: (easy)
In homework 1 you had the following problem:
   f(x) = 3x + 2
   g(x) = 2x + 3

   1. [3pt] What is the function composition,  (f o g) (x ) = ?
 
   2.   [3pt] What is  (g o f) (x ) = ?

   3.   [3pt] What is the value of  (f o g) (2 ) = ?

   4.   [3pt] What is the value of (g o f ) (2 ) = ?

Complete the haskell definition of f and g as described above.  
Then complete the definition for
      composefg 
so that it computes (f o g) and 
   composegf 
so that it computes  (g o f ).  
NOTICE that composefg and composegf do NOT have arguments. Does
the code produces the results calculated on homework 1.


> f x =
> g x =
> composefg =
> composegf =


What are the types of f, g, composefg, composegf?
