...
5)(Easy) Write a Haskell script using recursion, numToBoolR test lst, 
...
> pyth = [(x,y,z)| x <- [1..48], y <-[(x+1)..49],z<-[(y+1)..50], 
>                                        z*z == y*y +x*x]



> fastpyth x = [(a,b,c) | a <- [3..x], b <- [(+1)a..x], c <- [sqrt(a^2+b^2)], c == toEnum(floor(c)), c<= x]

> pythN x = [(a,b,c) | a<- [3..x], b <- [(+1)a..x], c <- [(+1)b..x], 0 == c^2 -a^2 -b^2]
...
> calPoly coef x = sum [a*x^e | (a,e)<- zip coef [0..]]
