...
void addBook(char * title, int year){
  /* This could be dangerous, the strings are NOT copied */
...
/* sortInventory uses the library supplied sorting program.
   qsort requires 
      ...
      a pointer to the code (bookcmp) used to induce an order on the data.  
            This code(bookcmp) will be bound at runtime.  

         Is this closer to the way Java works or 
	    to the way Templates work in C++?
