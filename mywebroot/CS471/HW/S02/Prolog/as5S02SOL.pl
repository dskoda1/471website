...
partition([],[],[]).
partition([H|T],[H|T1],T2):- partition(T,T2,T1).
...
% 3. oneString(lstOflstofStr, longString)

oneString([H|T],lStr):- oneString(H,hStr), oneString(T,tStr), append(hStr,tStr,lStr).
